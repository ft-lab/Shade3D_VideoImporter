...
レンダリングのタイムラインでの「開始フレーム」から「終了フレーム」以外のフレーム時は指定の「色」で塗りつぶします。    
...
「動画を変更」ボタンを押すと、mp4/webm、Windowsの場合はmov、を選択するファイルダイアログボックスが表示されます。    
マスターイメージに割り当てている動画は、ここで変更された動画に変更されます。    
...
「開始フレーム」と「終了フレーム」はレンダリングのタイムラインでのフレーム位置指定になります。    
...
「動画1」（ループ指定なし）の総フレーム数が100フレームの場合は、50+150=200フレームで動画の再生は停止されます。    
「動画2」（ループ再生）の総フレーム数が80フレームの場合は、79フレームが再生された後は0フレームに戻ります。    
