...
読み込んだ動画を削除するには、対象のマスターイメージをブラウザで選択して削除します。    

## 制限事項

* オーディオ情報は反映されません。    
* サードパーティ製レンダラプラグインなどでは、Video Importerプラグインで割り当てた動画ファイルかどうかを判断する手段がありません。<br>
これは、Shade3DプラグインSDK経由でマスターイメージを動画処理する機能にアクセスできないためです（カスタム動画を処理する機能自身がShade3D本体にないようです）。<br>
この場合は正しく動画レンダリングできない場合があります。    
